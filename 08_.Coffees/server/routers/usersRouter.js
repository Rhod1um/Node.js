...
//ses i node module, mappe i router under node modules har index.js ogs. express.js har route
//øverste import imporere det, neerste import peger på mappen
// se i node modules for ta se hvordan ting virker, koden er ikke svær vi kune have skrevet den selv

const router = Router()

router.use((req, res, next) => {
    req.specialString="moew"
    console.log(req.specialString)
    next()
})

//husk users i flertal fordi rest api
router.get("/users", (req, res) => {
    console.log(req.path, ":", req.session)
    console.log(process.env.SESSION_SECRET)
    console.log(req.specialString)  //special string kan ses her fordi det re langt i req, samme gør sesion middleware, 
    //lægger ting i req, godt med mmiddleware kan tjekke om man. fx databasekald i middleware og så får man id og så har man allerede id i routes fordi det blev gjort i middleware. sådan session fungere
